...
- **内置工具（84 个）**：
  ...
  - **原生视觉检测：`vision_inspect`** — macOS Vision 系统框架直接算出来的结果（**离线、零模型下载、毫秒级**）：图像分类、人脸/人体/人体姿势的**数量与像素位置**、动物、条码二维码、**视觉显著区域**。回答的是「图里有什么、在哪」这类**可判定**问题（框是像素坐标，可直接当 `image_inspect` 的 region 去核验）；语义描述才交给本地 VLM（`describe_image` 本机 30–120 秒且会幻觉），且先用它定位、**只裁局部**再喂
  ...
  - **代码执行（code-mode）：`run_code`** —— Agent 自己写 JS 做多步处理/批量计算/编排工具调用。
    在**可终止的沙箱 Worker** 里跑：没有文件/网络/系统能力，也没有 `window`，要读写文件或联网
    只能经 `tools.xxx()` 工具桥回到既有管线（权限矩阵 / 危险审批 / 审计照常生效）；
    单次 30 秒超时强制终止、工具调用上限 20 次。**默认关闭**，开关在「设置 → 权限」
...
- **代码执行沙箱** — `run_code` 默认关闭；开启后模型写的 JS 只在**可终止的 Worker** 里跑，
  没有文件/网络/系统能力与 `window`，也拿不到 Tauri IPC（无法绕过工具管线直接调命令）；
  能力只经 `tools.xxx()` 回到既有管线，30 秒超时强制终止
- **工作区 + 控制范围** — 输入框下工具栏的「工作区」按钮：设工作区目录（浏览/保存/清除，短名显示、
  悬浮看全路径），并用**档位**一键设定控制范围（借鉴 DSH 的权限预设：把「沙箱模式 + 危险命令审批」
  两个旋钮捆成一个选择器）：只读 / 工作区可写 / 工作区可写+智能审批 / 完全放开；组合不在表里时显示
  「自定义」。缺工作区时该档不生效（后端不会加沙箱）会**显式提示**，不留「以为受限、其实没有」的坑
- **命令沙箱（macOS Seatbelt）** — Agent 跑的命令可选三种限制：不改文件 / 只能写工作区；
  网络可另选三态：**不限 / 只通本机（断开外网）/ 完全断网**。覆盖 `run_command`、`exec_command`、
  `run_tests`、`git`；系统没有 `sandbox-exec` 时自动降级为不加沙箱（命令不会因此失败）
...
| 原生视觉检测 | macOS Vision（同一侧车：分类/人脸/人体/姿势/动物/条码/显著区域 + 图像特征指纹） |
| 图标 | lucide-vue-next |
