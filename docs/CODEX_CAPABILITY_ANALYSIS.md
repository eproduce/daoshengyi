...
> **进度：✅ 全部完成（2026-08-31）**——fork（db.rs `fork_conversation` + ChatHistory/消息「分支」按钮）+ queue（db.rs `append_message` + `queue_turn` 后台投递 + 事件刷新 + ChatHistory「投递」按钮）；resume 由点击会话续聊覆盖。Rust 测试 72 全绿。
...
## 6. Codex 工具面吸收对照（2026-09-15/16 逐 handler 核对）

> 依据 `codex-rs/core/src/tools/spec_plan.rs` 的 `add_core_tool_sources`（`add_shell_tools` /
> `add_mcp_resource_tools` / `add_core_utility_tools` / `add_collaboration_tools`）+ `hosted_model_tool_specs`
> + 扩展工具（goal / skills / web.run / imagegen），**逐个 handler 对照**。
> **结论：工具面已吸收约 80%（16 项），架构面（Code Mode / 多环境 / 暴露策略）未吸收；另有 2 项明确不做。**

### 6.1 已融合（16 项）

| Codex handler / spec | 道生一工具 | 说明 |
|---|---|---|
| `ApplyPatchHandler` | `apply_patch` | 自由格式补丁（Add/Update/Delete/Move to、`@@` 锚点、多 hunk），复用 diff 预览+确认+撤销栈 |
| `ExecCommandHandler` | `exec_command`（+ `run_command`） | PTY 启动 + 可续写；**超时不杀进程** |
| `WriteStdinHandler` | `write_stdin` | 向会话写输入 / 取增量输出，Ctrl-C 中断 |
| `ViewImageHandler` | `view_image` | 注入多模态消息；非多模态模型优先用云端视觉档，否则快速失败 |
| `CurrentTimeHandler` | `current_time` | 本地/指定时区 |
| `SleepHandler` | `sleep` | 0–60s 上限 |
| `PlanHandler` | `plan_task` / `plan_update` | 语义等价（**词表未对齐**：Codex 用 pending/in_progress/completed） |
| `GetContextRemainingHandler` | `get_context_remaining` | 用上轮真实 prompt tokens 计算占用 + 收尾建议 |
| `NewContextWindowHandler` | `new_context_window` | 历史压缩为摘要（`compactBefore/compactSummary`），只影响发给模型的历史 |
| `RequestPermissionsHandler` | `request_permissions` | 会话级授权（5 类能力），接入命令门禁 |
| `RequestUserInputHandler` | `request_user_input` | 应用内弹窗挂起等回答（可跳过 → 要求模型按假设继续） |
| `ListMcpResourcesHandler` + `ListMcpResourceTemplatesHandler` | `list_mcp_resources` | 两者**合并为一次调用**（资源 + 模板） |
| `ReadMcpResourceHandler` | `read_mcp_resource` | |
| `tool_search`（BM25 + deferred + `defer_loading`） | `tool_search` | 超预算工具进可检索目录，命中即激活（下一轮生效） |
| `multi_agents_v2::ListAgentsHandler` | `list_agents` | 后台子会话清单（状态/时长/结果预览） |
| `multi_agents_v2::InterruptAgentHandler`（含 v1 `CloseAgentHandler`） | `interrupt_agent` | 含 Rust `cancel_queued_turn` |
| `WebSearch`（hosted Responses 工具） | `web_search`（自研） | 有等价物 |

### 6.2 未吸收（按价值排序）

| Codex 能力 | 缺口本质 | 建议 |
|---|---|---|
| `CodeModeExecuteHandler` + `CodeModeWaitHandler`（codex-code-mode） | 模型**写代码**调用嵌套工具（循环/条件/一次多步），工具以代码 API 暴露 | ⚠️ **最大架构缺口**；需受限执行环境，工程量大 → 单独立项评估 |
| `multi_agents_v2::SendMessageHandler` / `FollowupTaskHandler` | 给**运行中**的子 agent 追加指令 / 给**已完成**的子 agent 派后续任务 | ✅ 建议补（小改动，可复用现有 `session_*` 基础设施） |
| `RequestUserInputAsyncHandler` / `SendMessageToUserAsyncHandler` | 异步提问、主动给用户发消息（不阻塞回合） | ⚠️ 已有 `notify_user` 系统通知（非模型工具）；异步提问需回合外状态机 |
| goal 三件套 `get_goal`/`create_goal`/`update_goal` | 跨回合**持久目标** + token 预算 | ⚠️ 与单回合 `plan_*` 互补；取决于是否要「长期目标」玩法 |
| `WaitForEnvironmentHandler` + 多 environment 抽象（工具带 `environment_id`） | 执行环境可为本机/容器/远程 | ❌ 不建议（单机桌面定位；`run_command`/`exec_command`/`sleep` 足够） |
| `DynamicToolHandler`（运行期注入工具定义） | 动态注册外部工具 | ➖ 已由 MCP 插件机制覆盖 |
| `ListAvailablePluginsToInstallHandler` / `RequestPluginInstallHandler` | 模型自行发现并**安装**插件 | ⚠️ 与既定方向冲突（插件收敛 + 市场降级）→ 建议「只提示不安装」 |
| 扩展 `skills.list` / `skills.read` | 技能成为可枚举/可读取的工具 | ➖ 我们「路由清单 + 命中注入」已达同等效果 |
| `TestSyncHandler` | 测试专用 | ➖ 不需要 |
| `imagegen`（`image_gen` 命名空间） | 文生图 | ⚠️ 本机无 GPU 不可本地跑；需接云端文生图 API |
| `ToolExposure::{Direct,Deferred,Hidden,CodeModeOnly}` + 命名空间合并 + `error_on_tool_collisions` | 工具暴露策略/命名空间分组（工具极多时省 schema 预算） | ➖ 已被 `tool_search` + 注册表目录部分覆盖；MCP 工具继续增长时再考虑命名空间 |

### 6.3 明确不做 / 不需要

- **自建插件 SDK、把插件市场当主入口**（方向已定：内置工具 + 技能 + MCP 长尾，见 `ROADMAP.md` §5.1）
- **多执行环境（容器 / 远程 workspace）**：与单机桌面定位不符
- Codex 的 Responses-Lite / Code-Mode 协议细节（我们走 OpenAI 兼容的 function calling）

### 6.4 反向：道生一有而 Codex 没有的

内置浏览器自动化（CDP，6 个工具）· 可视化工作流引擎 + 会话轨迹自动沉淀 · 知识库 RAG + 长期记忆/事实提取 ·
IM 网关 + 定时任务 · 系统托盘实时进度 + 系统通知 · 语音输入输出规划 · 国产模型适配（DeepSeek 计费/缓存口径）·
MCP 插件管理 UI 与远程 HTTP 插件 · 上下文预算三件套（占用查询/窗口压缩/产物折叠落盘）。

> **吸收是双向的**：工具面我们补得快（两天内 16 项），架构面（Code Mode）才是真正的深水区。
