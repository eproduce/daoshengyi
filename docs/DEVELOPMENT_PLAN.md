...
### 2.1b Puppeteer 多内核适配（✅ 已完成，2026-08-29；⚠️ WebKit/Safari 支持已取消）
- **目的**：从「只适配 Edge」升级为「按已安装浏览器 + 系统默认浏览器选择内核」——仅 Chromium 系（Chrome/Edge/Chromium/Brave），WebKit(Safari) 不做（Puppeteer 不支持，已取消预留）
...
- **表格/公式渲染**（ChatMessage.vue normalizeMath + wrapBareMathInTables）：`$` 内 `|` → `\\vert`（KaTeX）、`$` 外紧贴非空白的 `|` → `\\|`、列分隔符保留；全角竖线 `｜` → ` | `（两侧补空格）；**表格单元格裸数学自动包裹**（`x^n`/`1/x`/`√`/`∫` 等 → `$...$` 并转 KaTeX 命令）；系统提示引导（半角 `|`、数学用 `$...$`、单元格禁换行）
...
| # | 方向 | 复用基础 | 实现要点 |
|---|------|---------|---------|
| 1 | **会话内「撤销最近操作」** | ✅ | `apply_edits`（preview）+ `tool_audit`（工具调用全记录） | **已完成（2026-08-27）**：`undo_history` 表（action/path/backup/existed）+ `record_undo`（apply_edits 写盘前 / write_file_agent / delete_file_agent 自动快照）+ `undo_by_id`（edit 恢复内容 / create 删除新建 / delete 恢复文件，回滚后删记录）+ `list_undo`；命令 `apply_edits`/`delete_file_agent` 重构为纯函数+命令层（compute_edits/delete_file_impl）；前端 `UndoBubble` 右下角悬浮气泡显示最近可撤销操作一键回滚（写/删工具成功后 dispatch undo-changed 刷新）；测试 Rust +4 / 前端全绿。**操作回放面板 ✅ 已完成（2026-08-28）**：`UndoPanel.vue`（设置「撤销」Tab）——编辑/新建/删除快照列表 + 按路径/操作筛选 + 点击展开原始内容快照 + 一键回滚 + 导出 JSON |
| 2 | **项目语义索引 / 自然语言找代码**（P-A3 补全） | ✅ | `ollama_embed` + `kb_chunks` embedding 基建 | **已完成（2026-08-27）**：`code_chunks` 表（按 root 组织）+ `code_clear/code_add_chunk/code_search(余弦)/code_roots/code_stats`；命令 `code_index`（扫描代码文件→chunk_text 500 分块→Ollama 批量向量化，跳过 node_modules/.git/target 与大文件）/`code_search`（查询嵌入→余弦召回）/`code_roots`/`code_stats`/`code_delete`；前端内置工具 code_index/code_search/code_roots/code_stats/code_delete + BUILTIN_TOOLS/提示词描述；测试 Rust +1。**符号跳转 ✅ 已完成（2026-08-28）**：CodeChunkRow 加 `start_line`（code_search 实时算 chunk 在源文件的行号）+ `open_file` 支持 `line` 参数（优先 VSCode `code --goto path:line`，失败回退系统打开）+ 前端 code_search 结果带 `file:行号` + ChatMessage 链接识别 `:行号` 点击跳行。定义/引用跳转（tree-sitter 语义级）待做 |
...
| 5 | **工作流持久化 + 运行历史** | `workflow-engine` + 设置持久化 | **已完成（2026-08-27）**：`workflows` 表（name 唯一同名保存即更新）+ `workflow_runs` 表 + `wf_save/list/get/delete/run_add/runs`；命令 `workflow_*`；前端工具栏（名称保存 + 我的工作流下拉载入 + 删除当前 + 刷新）+ 运行成功/失败自动记录历史 + 底部「运行历史」列。**后续（同日）工作流 UI 优化**：接入 `WorkflowNodeView` 自定义节点（显式可拖拽 Handle 连接点，条件节点 T/F 双出点自动带分支标签）+ fit-view-on-init。**后续（08-30）拖拽新增修复 + 现代化**：useVueFlow 与 VueFlow 共享同 id store + `addNodes` 标准 API；WKWebView 下 HTML5 拖放 drop 不派发 → 改 mouse 事件自建拖拽（mousedown/move/up + 落点命中判定）+ 跟随鼠标的节点影像与画布高亮；共享色板 `data/workflow-colors.ts` 统一 palette 图例/节点/拖拽影像三处颜色；全套 `.wf-*` 样式主题化现代化
...
### 3.12.3 后续批次
- **第二批（🟡）**：
  - **S4 会话深度操作 ✅ 全部完成（2026-08-31）**：
    - **fork**：db.rs `fork_conversation`（全量复制 / 指定消息截断分支，新消息 id `{new_id}-{idx}` 防唯一冲突）+ `fork_conversation_cmd` + store `forkConversation` + ChatHistory 会话「分支」按钮 + ChatMessage 用户消息「从此分支」按钮。
    - **queue（异步投递）**：db.rs `append_message`（追加消息不清空、刷新 updated_at）+ `queue_turn` 命令（后台 `chat_once` 生成回复 → 追加 user/assistant 消息 → 事件 `queue-turn-done`/`queue-turn-error` 通知前端）+ store `queueTurn`/`refreshConversation` + ChatHistory 会话「投递」按钮 + 内联输入行。**坑**：后台任务不能持有 `State<Database>` 引用（生命周期逃逸 E0521），需独立 `Database::new(app_dir)`。
    - resume：历史会话续聊由「点击会话 → selectConversation」直接覆盖，无需单独入口。
...
| # | 能力 | 差距本质（道生一现状 → 目标机制） | 价值 | 优先级 | 状态 |
|---|------|--------------------------------|------|--------|------|
| O1 | 智能上下文压缩 | 「粗暴裁剪 120 万字符」→「LLM 提炼关键信息后压缩，替代丢弃」 | 高 | 🟢 | ✅ 2026-09-03 |
| O2 | SSRF 防护 | 「fetch_page/web_search 直连」→「hostname allowlist + 内网 IP 阻断」 | 高 | 🟢 | ✅ 2026-09-03 |
| O3 | 会话级工具集 | 「一次性子代理/fork/queue」→「agent 可 spawn/resume/yield 命名会话」 | 中 | 🟢 | ✅ 2026-09-03 |
| O4 | 入站内容安全边界 | 「外部内容直接注入」→「不可信内容包裹标记 + 防注入开关」 | 中 | 🟡 | ✅ 2026-09-04 |
