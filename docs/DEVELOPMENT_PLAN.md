...
| P-M2 | **并行子代理** | ⬜ | 🟢 | P-M1 | 多个子代理并发执行（前端 Promise.all / 后端 futures join），主代理分发子任务后并行收集结果；可视化面板显示各子代理运行状态 |
| P-M3 | **角色分工** | ⬜ | 🟡 | P-M2 | 面向任务的 agent 角色模板（规划者 / 执行者 / 验证者 / 评审者 / 研究助手），各自系统提示词 + 工具集约束 |
| P-M4 | **主代理汇总仲裁** | ⬜ | 🟡 | P-M3 | 主代理收集子代理结果后汇总 / 仲裁（冲突消解、交叉验证、统一呈现）；orchestrator-subagent 树 + 监视面板（ROADMAP Phase 4.4） |
