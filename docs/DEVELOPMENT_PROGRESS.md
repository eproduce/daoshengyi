...
### 当前状态（origin/main = `d63c322`，工作区干净）
- **内置工具 81 个**（本轮新增 `log_decision`）；原生 function calling + `tool_search` 渐进披露 + 巨型 schema 瘦身
- **测试**：vitest `29 files / 379 passed`；cargo `164 passed / 8 ignored`
...
- **可扩展/可控**：回收站删除 · 行锚点编辑 · 声明式权限规则 · 生命周期钩子 · 压缩阶梯 · 自动续跑规则表 · 决策日志 · 回复风格 · 技能外部导入 · 失败台账
