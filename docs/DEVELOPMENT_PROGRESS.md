...
### 当前状态（origin/main 已含 2026-09-24 修复：收尾轮关思考 + 截断续写 + 工具总次数，CI 绿）
...
- **长任务收尾不再「断了」**（2026-09-24 修复）：收尾轮关思考（`noThinking`）+ 收尾轮接上截断续写 +
  折叠头显示**总次数**（此前只有各工具名与 `×N`）——详见下方 09-24 一节
...
## 2026-09-24（修复：长任务「断了」+ 工具次数看不出来 —— 收尾轮被思考烧光输出预算）

用户实测（那条 54 次工具调用的会话）：「感觉内容乱了，工具调用次数统计，还有任务没有执行完断了」。

**诊断（看日志 + 拿真实数据跑真实代码，不猜）**：
- 日志：跑到**第 47 轮 / 54 次工具调用** → 正文空洞 → 追加收尾轮 → 收尾轮 `finish_reason=length`
  （8191 delta 撞输出上限）：**整个收尾轮 8192 token 全花在 reasoning**（累计 reasoning 17.3 万字符），
  正文为空 → 兜底把 reasoning 尾巴当答案（`（模型正文未输出实质内容，以下为思考摘要）` + 半句
  「…现在写最终回答。结构」）→ 看起来就是「乱 + 断了」
- **真实数据反证渲染层无错**：把库里那两条消息（54 张 / 31 张卡片）的正文喂给 `splitLeadingToolCards`
  （esbuild 把 TS 编成 mjs 跑），54/54、31/31 全部解析、剩余正文无卡片残留 → 不是折叠/反解的问题

**修法（三个点，对应三个症状）**：
1. **收尾轮关思考**：`streamRound` 新增 `noThinking`，收尾轮强制关掉——这一步只是「把已完成的结论
   写成正文」，思考没有增量价值，却是本次截断的直接原因（Rust 侧 `api.rs` 的 `thinking_enabled`
   是条件生效的，关掉真的会改变请求）
2. **截断续写抽成 `continueTruncated`，收尾轮也接上**：以前只有工具循环内读 `finish_reason`，
   收尾轮被截断就静默了（正是本次现象）；另补「正文为空」的专用续写指令（不是「紧接其后」）
3. **折叠头显示总次数**：以前只显示 `plan_task、pdf_read、plan_update ×2、+4`（`×N` 是**单个工具**的次数），
   看不出「总共调了多少次」→ 直接显示 `54 次`

**验证**：门禁 8/8（vitest 40 files / 528 passed，cargo 254 passed）。
...
**根因**：`.ci-drop` 是 `left: 0` 左对齐 + **无高度上限** → pill 靠窗口右侧时面板顶出右边，
窗口矮时顶出顶部（同一 bug 的两个症状）。浏览器实测复现：视口 780×560 时旧行为**溢出右边缘 86px**。

**改法（不写死方向，按几何算）**：
- 新增 `src/utils/dropdown-anchor.ts`（纯函数 + 5 单测）：`chooseDropdownAnchor`（右边放不下就
  向左展开；两边都放不下取**溢出更少**的一侧）+ `dropdownMaxHeight`（可用高度 = pill 顶部到视口顶部，
  但不低于 220px，超出就内部滚动）
- `ChatInput.vue`：打开面板后 `nextTick` 量 pill/面板尺寸算方向与高度上限；窗口 `resize` 时重算
  （仅面板开着时）；高度走内联 style，面板 `overflow-y: auto` + `overscroll-behavior: contain`
- 兜底：`.ci-drop { max-width: calc(100vw - 16px) }`——所有 pill 的下拉都不会比视口宽

**验证（浏览器实测，不是推理）**：
- 视口 780×560：面板 left=268 / right=608（贴 pill 右缘）、四边都在视口内、内部滚动生效；
  同一场景旧行为溢出右边缘 86px → 症状复现且已消除
- 开着面板改窗口尺寸：1400×900 → 自动切回左对齐（`panelLeft == pillLeft`）；缩回 780×560 → 切回右对齐
- 门禁 8/8（vitest **40 files / 515 passed**，cargo 251 passed）

**踩坑**：新写的 `chooseDropdownAnchor` 测试里，我把「两边都放不下」的期望值写反了
（该选溢出更少的一侧，实现是对的、测试是错的）——单测当场报错才发现，说明这条断言确实在起作用。
...
- **CI 教训（这次真踩了）**：GitHub 的 macOS runner 上 Apple Vision **拿不到推理上下文**
  （实测所有请求都报 `Could not create inference context`——无 GPU/ANE 的虚拟机）。
  原先两条真机断言按「本机有 GPU」写死（标签非空、距离为 0），CI 直接红（exit 101）。
  → 改为**优雅降级契约**断言（两边都必须成立）：能力不可用时侧车仍以 0 退出 + 给出可解析报告
  （尺寸照旧）+ 把**真实原因**写进 notes；相似度则退化为「可读错误」而不是空输出。
  另加报告级改进：全能力都没结果**且**有失败原因时，报告顶部出【警告】「这不是『图里什么都没有』」——
  否则模型会把「能力不可用」当成「图是空的」下结论。
  **另一条教训**：`gh run watch ... | tail` 会把退出码吃掉（管道最后一段的码），
  当时看到「退出码=0」其实 CI 是红的——要看 `gh run view --json conclusion` 或读 `--log-failed`。
...
（`snapshots/web/tool-details/ui.expected.md`：每次调用折叠成**一行摘要按钮 + 单独的 `Inspect`**，
默认只露摘要与少量关键字段）。

### 根因：同一次调用被显示了两遍

- `chat.ts` 轮末把**机器生成的卡片**拼进正文（`### 🔧 调用工具：\`x\`` + 参数/结果两个 `<details>`、
  `### 🌐 联网搜索` + 结果 `<details>`、失败单行 `> ❌ 工具调用失败`）——**刻意保留**：导出 Markdown
  与复制需要完整过程
- 同时 `assistantMsg.tools` 又渲染成结构化卡片 ⇒ 正文里一遍、卡片区一遍，且全部常显

### 改动

| 位置 | 改动 |
| --- | --- |
| `src/utils/tool-summary.ts`（新） | 纯函数 `summarizeTools`（次数 / 去重工具名 `web_search ×2、fetch_page、+1` / 总耗时 / 成败）与 `stripToolCards`（**仅显示层**剥掉开头连续的工具卡片；形状对不上立即停止，保守优先） |
| `ChatMessage.vue` | 工具区改为**整组可折叠**：摘要头（`▾ 工具调用 · 名字 · 全部成功 / 1 成功 · 1 失败 · 12.3s`）默认收起，展开后才是逐卡片（单卡仍可再展开看参数/结果）；`md()` 与流式正文都先 `stripToolCards`；**单卡展开状态改按索引**（原来按工具名，同一工具多次调用会「开一个全开」） |
| `chat.ts` | `toolCards` 改 `reactive` 并在轮内就挂到 `assistantMsg.tools` ⇒ **执行中分组能实时展开显示进度**（原来要等轮末才挂上） |

### 行为

- 执行中：分组自动展开（看得见 running 的工具），跑完自动收起；用户手动点过之后不再自动插手
- 有失败：摘要头边框变红 + 显示「N 成功 · M 失败」
- 流式期间正文只留占位提示（`🔧 正在调用工具：x…`），不再把整张卡片铺进气泡
- 导出 / 复制仍带完整工具过程（只改显示层，`assistantMsg.content` 不动）

### 验证

`npm run ci:local` 8/8 通过（vitest **37 files / 500 tests**，含新增 9 条：摘要去重计数、成败文案、
剥离 🔧/🌐/失败卡片、正文讲到卡片写法不误删、纯卡片→空串；cargo 224 passed / clippy 零告警）

---

## 2026-09-23（定时任务界面重做 + 立即运行 + 退出码状态）

用户反馈「定时任务设置界面让人不懂、感觉太简陋」。先做同类调研（本地 DSH 仓库
`docs/user/guide/schedule.zh.md`、`docs/subsystems/schedule.zh.md`、`2026-08-05-durable-web-schedule` 设计 note），
结论与取舍见 `docs/SCHEDULED_TASK_DESIGN.md`。

**调研要点**（DSH = 成熟 agent 产品的做法）：任务内容 = **提示词**、交付 = **普通对话轮次**
（无独立回执卡片）、调度原语只三种（延时/绝对时刻/固定速率）且**刻意不做 cron**、
时间输入必须**显式带时区**（DST 缺口拒绝、重叠取较早）、逾期**只补最新一次**绝不积压回放、
界面只读（列表 + 相对时间 + 状态），创建编辑交给模型工具。

### 界面重做（`src/components/ScheduledTasks.vue`）

| 改动 | 说明 |
| --- | --- |
| 两段式表单 | **① 跑什么**（名称 + 命令）／**② 什么时候跑**（预设 + 自定义），不再是一排裸输入框 |
| 命令示例 chips | 磁盘占用 / 堵在前 10 的进程 / 备份目录 / 清理 7 天前日志 / 联网自检，点一下填进命令框 |
| 调度预设 chips | 每 15 分钟 ~ 每天 22:00 共 7 个，命中当前配置时高亮 |
| 下次运行实时预览 | `09-24 09:00（8 小时后）`——创建/编辑时就能看懂「它什么时候会跑」 |
| 任务卡片 | 状态图标（成功/失败/超时/未运行）+ 调度摘要 + 下次时间与**倒计时（逾期高亮）** + 上次运行时间 + 输出**折叠**（不再铺开 1000 字符） |
| 立即运行 | 验证配置用：跑一次并回显退出码/耗时/输出，**不动下次运行时间** |
| 编辑 | 复用同一套表单预填（原来只能删了重建） |
| 弹窗统一 | 全部走 `notify` / `askConfirm`（WKWebView 下 alert/confirm 有已知问题） |
| 主题 | 所有按钮显式 `color` + 主题变量（修掉暗色下按钮黑字看不见的隐患） |

### 纯逻辑抽出（`src/utils/schedule-format.ts` + 9 条单测）

间隔人话化（60→每小时、1440→每天）、相对时间、状态判定（**退出码优先**，老数据回退文本启发式）、
预设匹配与套用、下次运行计算（与 Rust `compute_next_run` 同语义：daily 已过/恰好等于→次日；
interval ≤0→60 分钟）、表单校验。

### 后端

- `scheduled_tasks` 新增 `last_exit_code`（建表 + **幂等迁移** + 行映射 + 读写；2 条 db 测试：
  退出码往返、**旧库自动补列**）
- 新命令 `run_scheduled_task_now`（立即运行一次）
- **执行路径统一**为 `exec_scheduled_command` → 复用 `run_shell_command_with`
  （并发读 stdout/stderr、**300 秒超时并杀掉整个进程组**、shell 语义）

> ⚠️ 顺带修掉一个真实隐患：原调度器直接 `Command::new("/bin/sh")` 且**没有超时**——一条卡住的命令
> 会把整条调度线程堵死（之后所有定时任务不再执行），而代码注释却写着「300 秒超时」。
> 现在超时是真的，且与 `run_command` 共用同一条执行路径。

### 验证

`npm run ci:local` 8/8 通过（vitest 全绿 + `cargo test --lib` **224 passed** / 13 ignored + clippy `-D warnings` 零告警）

### 待办（见 `docs/SCHEDULED_TASK_DESIGN.md` §3）

①任务内容支持**提示词**（让 agent 干活）②结果**对话式交付**（投回会话）③新增**一次性**原语
④时区显式化 ⑤逾期汇总提示。**明确不做**：cron/日历表达式（与 DSH 同判断）。
