//! 多源网络搜索：百度 / 必应 / 360 / 搜狗 并行综合（国内直连可用）。
//!
//! 按 域名+路径 去重合并（每源限 5 条、总上限 15），内置反爬静默降级（单源被反爬不影响其它）、
//! 相关性过滤（剔除词典释义 / 股票 / 问答噪声）；全空时兜底 DuckDuckGo。
...
fn filter_relevant(query: &str, results: &[SearchResult]) -> Vec<SearchResult> {
    let query_terms: Vec<String> = cjk_terms(query)
        .split_whitespace()
        .map(|s| s.to_string())
        .collect();
...
async fn search_baidu(query: &str) -> Result<Vec<SearchResult>, String> {
        ...
        .await
    {
        Ok(r) => match r.text().await {
            Ok(t) => t,
            Err(e) => return Err(format!("百度读取失败: {}", e)),
        },
...
fn parse_baidu(html: &str) -> Vec<SearchResult> {
    ...
    while results.len() < 10 {
        ...
        let end = match html[start + 5..].find("class=\"result ") {
            Some(i) => start + 5 + i,
            None => cbound(html, (start + 8000).min(html.len())),
        };
...
fn extract_baidu_mu(block: &str) -> String {
    ...
    while let Some(i) = block[search_from..].find("mu=\"") {
        ...
        if url.contains("baidu.com/link")
            || url.contains("recommend_list")
            || url.contains("nourl.ubs")
            || url.contains("top.baidu.com")
            || url.contains("aiqicha.baidu.com")
            || url.contains("passport.baidu.com")
            || url.contains("baidu.com/s?")
        {
            continue;
        }
...
fn extract_baidu_summary(block: &str) -> String {
    let sd = match block.find("<!--s-data:") {
        Some(i) => i + 11,
        None => return String::new(),
    };
    let ed = match block[sd..].find("-->") {
        Some(i) => sd + i,
        None => return String::new(),
    };
    ...
    let json_clean = json_str
        ...
        .replace("&lt;", "<")
        .replace("&gt;", ">");
    let v: serde_json::Value = match serde_json::from_str(&json_clean) {
        Ok(v) => v,
        Err(_) => return String::new(),
    };
    let mut out = String::new();
    if let Some(lines) = v
        .pointer("/summaryData/generalLines")
...
async fn search_360(query: &str) -> Result<Vec<SearchResult>, String> {
        ...
        .await
    {
        Ok(r) => match r.text().await {
            Ok(t) => t,
            Err(e) => return Err(format!("360 读取失败: {}", e)),
        },
    ...
    if !html.contains("res-list")
        && (html.len() < 3000 || anti_spider.iter().any(|k| html.contains(k)))
    {
    ...
    while results.len() < 10 {
        ...
        let end = match html[start + 12..].find("class=\"res-list\"") {
            Some(i) => start + 12 + i,
            None => cbound(&html, (start + 3000).min(html.len())),
        };
        let block = &html[start..end];
...
async fn search_sogou(query: &str) -> Result<Vec<SearchResult>, String> {
        ...
        .await
    {
        Ok(r) => match r.text().await {
            Ok(t) => t,
            Err(e) => return Err(format!("搜狗读取失败: {}", e)),
        },
    ...
    while results.len() < 10 {
        ...
        let end = match html[start + 5..].find("class=\"rb\"") {
            Some(i) => start + 5 + i,
            None => cbound(&html, (start + 2500).min(html.len())),
        };
        let block = &html[start..end];
        ...
        if !title.is_empty() {
            ...
            if href.starts_with('/') {
                href = format!("https://www.sogou.com{}", href);
            }
...
fn extract_vr_title(block: &str) -> String {
    // 在 h3 范围内找 <a ...>，取其闭合前文本
    let h3_start = match block.find("class=\"vr-title") {
        Some(i) => i,
        None => return String::new(),
    };
    let seg = &block[h3_start..];
    let a_start = match seg.find("<a") {
        Some(i) => i,
        None => return String::new(),
    };
    let a_gt = match seg[a_start..].find('>') {
        Some(i) => a_start + i + 1,
        None => return String::new(),
    };
    let a_end = match seg[a_gt..].find("</a>") {
        Some(i) => a_gt + i,
        None => return String::new(),
    };
...
async fn search_bing(query: &str) -> Result<Vec<SearchResult>, String> {
    ...
    for domain in ["cn.bing.com", "www.bing.com"] {
        let url = format!(
            "https://{}/search?q={}&count=10&setlang=zh-cn",
            domain,
            urlencoding(query)
        );
        let client = reqwest::Client::builder()
        ...
        let html = match client.get(&url).send().await {
            Ok(r) => match r.text().await {
                Ok(t) => t,
                Err(e) => {
                    last_err = format!("必应 {} 读取失败: {}", domain, e);
                    continue;
                }
            },
            Err(e) => {
                last_err = format!("必应 {} 连接失败: {}", domain, e);
                continue;
            }
        };
...
fn extract_h2(block: &str) -> String {
    let open_start = match block.find("<h2") {
        Some(i) => i,
        None => return String::new(),
    };
    let gt = match block[open_start..].find('>') {
        Some(i) => open_start + i + 1,
        None => return String::new(),
    };
    let close = match block[gt..].find("</h2>") {
        Some(i) => gt + i,
        None => return String::new(),
    };
...
fn extract_bing_caption(block: &str) -> String {
    let cap_start = match block.find("class=\"b_caption\"") {
        Some(i) => i,
        None => return extract_tag(block, "<p", "</p>"),
    };
    let after = &block[cap_start..];
    let p_start = match after.find("<p") {
        Some(i) => i + 2,
        None => return String::new(),
    };
    let p_gt = match after[p_start..].find('>') {
        Some(i) => p_start + i + 1,
        None => return String::new(),
    };
    let p_end = match after[p_gt..].find("</p>") {
        Some(i) => p_gt + i,
        None => return String::new(),
    };
...
fn parse_ddg(html: &str) -> Vec<SearchResult> {
    ...
    while results.len() < 8 {
        ...
        let a_start = match html[..start].rfind("<a") {
            Some(i) => i,
            None => break,
        };
        let href = extract_attr(&html[a_start..start], "href=\"", "\"");
        let title_end = match html[start..].find("</a>") {
            Some(i) => start + i,
            None => break,
        };
...
fn extract_attr(html: &str, attr: &str, quote: &str) -> String {
    let start = match html.find(attr) {
        Some(i) => i + attr.len(),
        None => return String::new(),
    };
    let end = match html[start..].find(quote) {
        Some(i) => start + i,
        None => return String::new(),
    };
...
mod tests {
    ...
    fn test_percent_decode() {
        assert_eq!(
            percent_decode("https%3A%2F%2Fwww.huawei.com%2Fcn%2F"),
    ...
    async fn test_duckduckgo_live() {
        let r = search_duckduckgo("华为技术有限公司")
            .await
            .expect("DDG 请求失败");
        assert!(!r.is_empty(), "DDG 无结果");
        assert!(
            !r[0].title.is_empty() && r[0].url.starts_with("http"),
            "结果格式异常: {:?}",
            r[0]
        );
    ...
    fn test_baidu_parser_synthetic() {
        ...
        assert!(
            r[0].snippet.contains("华为创立于1987年"),
            "摘要应来自 s-data: {}",
            r[0].snippet
        );
    }
    ...
    fn test_dedup_key() {
        ...
        assert_eq!(
            dedup_key("https://example.com/path#frag"),
            "example.com/path"
        );
    ...
    fn test_filter_relevant_keeps_matching_drops_unrelated() {
        let results = vec![
            SearchResult {
                title: "什么是人工神经网络 - 知乎".into(),
                url: "https://zhihu.com/a".into(),
                snippet: "人工神经网络由大量神经元组成".into(),
            },
            SearchResult {
                title: "Microsoft Corporation (MSFT) Stock".into(),
                url: "https://finance.yahoo.com/quote/MSFT".into(),
                snippet: "stock price news".into(),
            },
            SearchResult {
                title: "深度学习与神经网络入门".into(),
                url: "https://example.com/b".into(),
                snippet: "神经网络是深度学习的基础".into(),
            },
        ];
        ...
        assert_eq!(
            kept.len(),
            2,
            "应剔除无关的 MSFT 股票结果，保留中文相关结果: {:?}",
            kept.iter().map(|r| &r.title).collect::<Vec<_>>()
        );
        assert!(
            kept.iter().any(|r| r.url.contains("zhihu")),
            "应保留知乎结果"
        );
    ...
    fn test_filter_relevant_english_case_insensitive() {
        let results = vec![
            SearchResult {
                title: "DeepSeek V4 model".into(),
                url: "https://x.com".into(),
                snippet: "deepseek".into(),
            },
            SearchResult {
                title: "无关内容".into(),
    ...
    fn test_filter_relevant_removes_noise() {
        let results = vec![
            // 命中查询但属词典释义噪声 → 应剔除
            SearchResult {
                title: "人工的意思_人工的解释".into(),
                url: "https://hanyu.com".into(),
                snippet: "人工 的意思".into(),
            },
            // 命中查询的真实结果 → 应保留
            SearchResult {
                title: "人工神经网络入门教程".into(),
                url: "https://real.com".into(),
                snippet: "人工神经网络".into(),
            },
            // 股票噪声 → 应剔除
            SearchResult {
                title: "Microsoft (MSFT) stock quote".into(),
                url: "https://finance.yahoo.com/quote/MSFT".into(),
                snippet: "microsoft stock price".into(),
            },
        ];
    ...
    async fn diag_ai_search() {
        ...
        for x in r.iter().take(10) {
            eprintln!(
                "  [{}] {}",
                x.title.chars().take(40).collect::<String>(),
                x.url.chars().take(55).collect::<String>()
            );
        }
    }
}
