...
const filtered = computed(() => {
  ...
  return rows.value.filter((r) => {
    ...
    if (kw && !r.tool_name.toLowerCase().includes(kw) && !r.arguments.toLowerCase().includes(kw))
      return false;
    return true;
  });
});
...
function exportMd() {
  ...
  for (const r of filtered.value.slice(0, 500)) {
    lines.push(
      `| ${r.id} | ${r.tool_name} | ${fmtTime(r.created_at)} | ${fmtDur(r.duration_ms)} | ${r.is_error ? "❌失败" : "✅成功"} | ${escapeMd(r.arguments.slice(0, 80))} | ${escapeMd(r.result.slice(0, 80))} |`,
    );
  }
  ...
  <div class="ap-panel">
    ...
    <div class="ap-list">
      <div
        ...
        :key="r.id"
        class="ap-row"
        :class="{ err: r.is_error, open: expandedId === r.id }"
        @click="toggle(r.id)"
      >
        ...
        <div v-if="expandedId === r.id" class="ap-row__detail" @click.stop>
          <div class="ap-detail-block">
            <b>参数</b>
            <pre class="ap-pre">{{ prettyJson(r.arguments) }}</pre>
          </div>
          <div class="ap-detail-block">
            <b>结果</b>
            <pre class="ap-pre" :class="{ err: r.is_error }">{{ r.result }}</pre>
          </div>
        </div>
      </div>
