  ...
  <div class="history-panel">
    <div class="history-panel__header">
      ...
      <div class="history-panel__acts">
        <button v-if="!showArchived" class="btn-icon" title="查看归档" @click="showArchived = true">
          <Archive :size="15" />
        </button>
        <button v-else class="btn-icon" title="返回对话列表" @click="showArchived = false">
          ←
        </button>
        <button
          v-if="!showArchived"
        ...
        >
          ...
          <div class="history-item__btns">
            <button
              ...
              title="投递任务（后台执行）"
              @click.stop="toggleQueue(conv.id)"
            >
              <Send :size="14" />
            </button>
            <button
              class="history-item__btn"
              title="分支（复制为新对话）"
              @click.stop="chatStore.forkConversation(conv.id)"
            >
              <GitBranch :size="14" />
            </button>
            <button
              class="history-item__btn"
              title="导出为 Markdown"
              @click.stop="chatStore.downloadExport(conv.id, 'md')"
            >
              ⤓
            </button>
            <button
              class="history-item__btn"
              title="归档（隐藏，可恢复）"
              @click.stop="chatStore.archiveConversation(conv.id)"
            >
              <Archive :size="14" />
            </button>
            <button
          ...
          <div v-if="queueTarget === conv.id" class="history-item__queue" @click.stop>
            <input
              v-model="queueText"
              class="history-item__queue-input"
              placeholder="投递给此会话的任务内容…"
              @keyup.enter="submitQueue(conv.id)"
            />
        ...
        >
          ...
          <div class="history-item__btns">
            ...
            <button
              class="history-item__btn"
              title="恢复（移回主列表）"
              @click.stop="chatStore.unarchiveConversation(conv.id)"
            >
              ↩
            </button>
            <button
              class="history-item__delete"
              title="彻底删除"
              @click.stop="chatStore.deleteArchived(conv.id)"
            >
              ✕
            </button>
          </div>
...
.history-item__queue-input:focus {
