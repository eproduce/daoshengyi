...
import { normalizeMath } from "@/utils/markdown-math";
...
// 数学公式与表格预处理已抽取到 src/utils/markdown-math.ts（normalizeMath）
// 见 md() 调用；测试：node scripts/test-markdown-math.mts
...
marked.use({
  tokenizer: {
    url(src: string) {
      const cap = /^https?:\/\/[^\s<]*[^\s<,.:;"')\]\u4e00-\u9fa5\u3000-\u303f\uff00-\uffef]/u.exec(
        src,
      );
      if (cap) {
        const text = cap[0];
...
async function copyAll() {
  await chatStore.copyToClipboard(props.message.content);
  copied.value = true;
...
onMounted(() => {
  if (props.message.content && !props.message.streaming) highlighted = false;
});
onUpdated(() => {
...
<template>
  ...
  <div v-else class="message" :class="`message--${message.role}`" :data-msg-id="message.id">
    <div class="message__avatar">
      <User v-if="message.role === 'user'" :size="18" /><AppLogo v-else :size="24" />
    </div>
    <div class="message__body">
      <div class="message__bubble">
        ...
        <div v-if="message.attachments?.length" class="message__attachments">
          <div
            v-for="f in message.attachments"
            :key="f.id"
            class="attach-card"
            :title="`${f.name} · ${(f.content || '').length} 字符`"
          >
      ...
      <div class="message__meta">
        ...
        <div v-if="message.role === 'assistant' && message.content" class="msg-actions">
          <button class="msg-act-btn" @click="copyAll">
            <Copy v-if="!copied" :size="14" />{{ copied ? "已复制" : "复制" }}
          </button>
          <button class="msg-act-btn" @click="chatStore.retryLast()">
            <RotateCw :size="14" /> 重试
          </button>
        </div>
        <div v-else-if="message.role === 'user' && message.content" class="msg-actions">
          ...
          <button class="msg-act-btn" title="从此消息分支为新对话" @click="forkFromThis">
...
.message--user .message__bubble .message__content {
  color: #fff;
}
...
.message__image-item:hover {
  transform: scale(1.03);
}
.message__image-item img {
  width: 100%;
  height: 100%;
  object-fit: cover;
}
...
.message--user .attach-card {
  background: rgba(255, 255, 255, 0.18);
  border-color: rgba(255, 255, 255, 0.3);
}
.message--user .attach-card-name {
  color: #fff;
}
.message__content {
  font-size: 14px;
...
.message--user .message__content .local-file-link {
  color: #fff;
}
.message__cursor {
...
.terminal-card__dot.r {
  background: #ff5f57;
}
.terminal-card__dot.y {
  background: #febc2e;
}
.terminal-card__dot.g {
  background: #28c840;
}
.terminal-card__cmd {
  font-size: 11px;
