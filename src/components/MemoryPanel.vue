...
const stats = computed(() => {
  ...
  return {
    total: facts.value.length,
    avgImportance: facts.value.length ? (totalImportance / facts.value.length).toFixed(1) : "0",
    byType,
  };
});
...
// --- 编辑记忆 ---
const editing = ref<{ id: string; fact: string; fact_type: string; importance: number } | null>(
  null,
);
...
<template>
  <div class="memory-panel">
    <p class="memory-desc">
      长期记忆：对话自动提取 +
      跨会话检索注入。系统每天自动做<strong>重要度衰减</strong>与<strong>低价值遗忘</strong>（偏好永久保留）。
    ...
    <div class="memory-config">
      <label class="memory-config__toggle">
        <input v-model="memoryEnabled" type="checkbox" @change="saveMemoryConfig" />
        <span>启用记忆注入</span>
      </label>
      <label class="memory-config__limit">
        相关记忆检索条数
        <input
          v-model="memoryRecallLimit"
          type="number"
          min="1"
          max="20"
          @change="saveMemoryConfig"
        />
      </label>
    </div>
    ...
    <div class="memory-toolbar">
      ...
      <button class="memory-btn" :disabled="loading" @click="load">
        <RefreshCw :size="14" :class="{ spinning: loading }" /> 刷新
      </button>
      <button class="memory-btn" @click="runMaintenance"><Sparkles :size="14" /> 执行维护</button>
      <button
        class="memory-btn"
        :disabled="!!reviewMsg && reviewMsg === '复习中…'"
        @click="runReview"
      >
        <Brain :size="14" /> 智能复习
      </button>
      <button
        class="memory-btn"
        :disabled="!!episodicMsg && episodicMsg === '汇总中…'"
        @click="runAggregate"
      >
    ...
    <div class="memory-stats">
      <span
        ><strong>{{ stats.total }}</strong> 条事实</span
      >
      <span
        >平均重要度 <strong>{{ stats.avgImportance }}</strong></span
      >
    ...
    <div v-if="forgetCandidates.length > 0" class="memory-candidates">
      <h4>
        <Trash2 :size="14" /> 遗忘候选（{{ forgetCandidates.length }}）— 低重要度且 30 天以上未访问
      </h4>
    ...
    <div class="memory-list">
      <div v-if="filteredFacts.length === 0 && !loading" class="memory-empty">
        {{ facts.length === 0 ? "暂无记忆（对话结束后自动提取）" : "没有匹配的搜索词" }}
      </div>
      <div
        v-for="f in filteredFacts"
        :key="f.id"
        class="memory-item"
        :class="`memory-item--${f.fact_type}`"
      >
        <div class="memory-item__head">
          ...
          <span class="memory-importance" :title="`重要度 ${f.importance}/10`">
            {{ "★".repeat(Math.min(f.importance, 5))
            }}<span class="dim">{{ f.importance > 5 ? "★".repeat(f.importance - 5) : "" }}</span>
          </span>
          <span class="memory-meta"
            >访问 {{ f.access_count }} 次 · {{ fmtTime(f.last_accessed || f.created_at) }}</span
          >
          <button class="memory-del" title="编辑这条记忆" @click="startEdit(f)">
            <Pencil :size="13" />
          </button>
          <button class="memory-del" title="删除这条记忆" @click="deleteFact(f.id)">
            <Trash2 :size="13" />
          </button>
        </div>
        <div v-if="editing && editing.id === f.id" class="memory-edit">
          ...
          <div class="memory-edit__row">
            <select v-model="editing.fact_type" class="memory-filter">
              <option v-for="t in ['preference', 'info', 'decision', 'todo']" :key="t" :value="t">
                {{ TYPE_LABEL[t] }}
              </option>
            </select>
            <input
              v-model.number="editing.importance"
              type="number"
              min="1"
              max="10"
              class="memory-edit__imp"
              title="重要度 1-10"
            />
    ...
    <div class="memory-episodic">
      ...
      <p class="memory-episodic__hint">
        把多段会话摘要汇总成反复出现的主题/项目/持续关注点。点「跨会话汇总」用模型生成；语义层：事实=memory_facts、单会话摘要=memory_summaries、此处=跨会话聚合。
      </p>
      <div v-if="episodic.length === 0" class="memory-episodic__empty">
        尚未生成跨会话主题。先让多个对话生成会话摘要，再点上方「跨会话汇总」。
      </div>
      <div v-for="e in episodic" :key="e.id" class="memory-episodic__item">
        <div class="memory-episodic__head">
          ...
          <button class="memory-del" title="删除该主题" @click="deleteEpisodic(e.id)">
            <X :size="13" />
          </button>
        </div>
...
.memory-profile__items {
  display: flex;
  flex-wrap: wrap;
  gap: 6px;
}
.memory-profile__chip {
  font-size: 12px;
  padding: 3px 8px;
  border-radius: 12px;
  background: var(--bg-input);
...
.memory-episodic__hint {
  font-size: 11px;
  color: var(--text-secondary, #888);
  margin: 0 0 8px;
  line-height: 1.5;
}
.memory-episodic__empty {
  font-size: 12px;
  color: var(--text-secondary, #888);
  padding: 6px 0;
}
.memory-episodic__item {
  border: 1px solid #6a1b9a33;
...
.memory-episodic__head {
  display: flex;
  align-items: center;
  gap: 8px;
  margin-bottom: 4px;
}
.memory-episodic__title {
  font-size: 13px;
  color: #6a1b9a;
}
.memory-episodic__meta {
  font-size: 11px;
  color: #999;
  flex: 1;
}
.memory-episodic__text {
  font-size: 12px;
...
.memory-config__toggle {
  display: inline-flex;
  align-items: center;
  gap: 6px;
  cursor: pointer;
}
.memory-config__limit {
  display: inline-flex;
  align-items: center;
  gap: 6px;
}
.memory-config__limit input {
  width: 56px;
  padding: 3px 6px;
  border-radius: 6px;
  border: 1px solid var(--border, #ddd);
...
.memory-candidate__text {
  flex: 1;
}
.memory-candidate__del {
  border: none;
  background: none;
  cursor: pointer;
  color: #bbb;
