...
<template>
  <div class="pty-panel">
    ...
    <p class="ollama-desc">
      启动长驻/交互式进程（dev server、REPL、watch 等），在此实时查看输出并输入指令。命令用
      <code>sh -c</code> 执行，支持管道 / 重定向 / 环境变量。
    </p>
    ...
    <div class="pty-form">
      <input
        v-model="cmd"
        class="pty-input"
        placeholder="命令，如：npm run dev、python3、node -i"
        @keyup.enter="doSpawn"
      />
      <input
        v-model="cwd"
        class="pty-input"
        placeholder="工作目录（留空=默认）"
        @keyup.enter="doSpawn"
      />
      <button class="btn-secondary" :disabled="pty.busy" @click="doSpawn">
        <Play :size="14" /> 启动
      </button>
    </div>
    ...
    <div v-if="pty.sessions.length" class="pty-sessions">
      ...
      >
        ...
        <span class="pty-session-tab__cmd"
          >{{ s.command.slice(0, 26) }}{{ s.command.length > 26 ? "…" : "" }}</span
        >
        <button class="pty-session-tab__kill" title="终止" @click.stop="pty.kill(s.id)">
          <Square :size="12" />
        </button>
      </div>
    </div>
    ...
    <div v-if="active" class="pty-terminal">
      <pre ref="outputEl" class="pty-output" @scroll="onScroll">{{
        active.output || "（等待输出…）"
      }}</pre>
      <div class="pty-input-row">
        <input
          ...
          @keyup.enter="doSend"
        />
        <button class="btn-secondary" :disabled="!active.running || !input" @click="doSend">
          发送
        </button>
        <button
          class="btn-ghost"
          :disabled="!active.running"
          @click="active && pty.kill(active.id)"
        >
          <Trash2 :size="14" /> 终止
        </button>
      </div>
...
.pty-session-tab--active {
  border-color: var(--accent-color);
  background: var(--accent-bg);
}
.pty-session-tab__status {
  width: 8px;
  height: 8px;
  border-radius: 50%;
  background: var(--border-color);
}
.pty-session-tab__status.is-running {
  background: #22c55e;
}
.pty-session-tab__kill {
  background: none;
  border: none;
  color: var(--text-secondary);
  cursor: pointer;
