...
<template>
  <div class="quick-bar">
    ...
    >
      ...
      <span class="thinking-toggle__label">{{
        chatStore.activeProfile?.thinkingEnabled ? "深度思考" : "快速回答"
      }}</span>
    </button>
    ...
    <div v-if="showDropdown" ref="dropdownRef" class="quick-bar__dropdown">
      ...
      >
        ...
        <div class="quick-bar__item-detail">{{ profile.model }} · {{ profile.baseUrl }}</div>
      </div>
      <div class="quick-bar__dropdown-footer">
        <button
          class="quick-bar__settings-btn"
          @click="
            emit('openSettings');
            showDropdown = false;
          "
...
.quick-bar__item:first-child {
  border-radius: var(--radius-lg) var(--radius-lg) 0 0;
}
.quick-bar__item:hover {
  background: var(--bg-hover);
}
.quick-bar__item--active {
