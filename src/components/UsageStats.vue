...
const aggTotal = computed(() => ({
  ...
  duration:
    chat.usageAgg?.total?.total_duration ??
    totalStats.value.avgDuration * totalStats.value.messages,
}));
...
<template>
  <div class="usage-panel">
    ...
    <div class="usage-block">
      ...
      <div v-if="byDay.length" class="bar-chart" :style="{ height: '120px' }">
        <div
          ...
          :title="`${d.date}：${d.tokens} token / ${d.count} 条`"
        >
          <div
    ...
    <div class="usage-block">
      <div class="usage-block__title">
        会话 Token 分布 <span class="usage-muted">（Top {{ topConvs.length }}）</span>
      </div>
      <div v-if="topConvs.length" class="conv-bars">
        <div
          ...
          :key="c.id"
          class="conv-row"
          :title="`${c.title}：${c.tokens} token / ¥${c.cost.toFixed(4)} / ${c.msgs} 条`"
        >
          <div class="conv-row__name">{{ c.title }}</div>
          <div class="conv-row__track">
            <div
              class="conv-row__fill"
              :style="{ width: (c.tokens / maxConvTokens) * 100 + '%' }"
            ></div>
          </div>
...
.bar-chart__bar {
  width: 70%;
  background: linear-gradient(180deg, #7c6cff, #5a4bdb);
