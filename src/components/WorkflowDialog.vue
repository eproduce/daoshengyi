...
const runs = ref<
  { id: number; wf_name: string; status: string; started_at: number; summary: string }[]
>([]);
...
const selectedEdge = computed<any>(() =>
  selectedEdgeId.value
    ? (edges.value as { id: string }[]).find((e) => e.id === selectedEdgeId.value)
    : undefined,
);
...
function addNode(type: WorkflowNodeType, pos?: { x: number; y: number }) {
  ...
  const wf: WorkflowNode = {
    id,
    type,
    label: `${TYPE_LABEL[type]}节点`,
    config:
      type === "llm"
        ? { prompt: "请基于上方上下文回答：{{user}}" }
        : type === "tool"
          ? { tool: "web_search", toolArgs: { query: "{{user}}" } }
          : type === "text"
            ? { text: "输入内容" }
            : type === "condition"
              ? { expression: '{{user}} != ""' }
...
async function run() {
  ...
  for (const n of nodes.value) {
    (n.data as any).wf.runStatus = "waiting";
    (n.data as any).wf.runOutput = "";
  }
...
async function refreshWorkflows() {
  try {
    myWorkflows.value =
      await invoke<{ id: number; name: string; updated_at: number }[]>("workflow_list");
    runs.value = await invoke<
      { id: number; wf_name: string; status: string; started_at: number; summary: string }[]
    >("workflow_runs", { limit: 10 });
  } catch {
    /* 后端暂不可用 */
  }
}
async function saveWorkflow() {
  ...
  if (!name) {
    log.value = ["请先在名称框输入工作流名称再保存"];
    return;
  }
  const graph = buildGraph();
  try {
    loadedWfId.value = await invoke<number>("workflow_save", {
      name,
...
async function loadWorkflow(id: number) {
  try {
    const w = await invoke<{ id: number; name: string; graph: string } | null>("workflow_get", {
      id,
    });
    if (!w) return;
...
function exportJson() {
  ...
  a.href = URL.createObjectURL(
    new Blob([JSON.stringify(buildGraph(), null, 2)], { type: "application/json" }),
  );
...
<template>
  <div class="wf-overlay">
    ...
    <div
      v-if="dragPreview"
      class="wf-drag-preview"
      :style="{
        '--t': WORKFLOW_NODE_COLORS[dragPreview.type],
    ...
    <div class="wf-dialog">
      <header class="wf-head">
        ...
        <div class="wf-head__actions">
          <button class="wf-btn wf-btn--run" :disabled="running" @click="run">
      ...
      <div class="wf-toolbar">
        <input
          ...
          placeholder="工作流名称（保存用）"
          @keydown.enter="saveWorkflow"
        />
        <button class="wf-btn" @click="saveWorkflow"><Save :size="13" /> 保存</button>
        <select
          class="wf-palette__select"
          @change="
            (e: any) => {
        ...
        >
          ...
          <option v-for="w in myWorkflows" :key="w.id" :value="w.id">
            {{ w.name }}（{{
              w.updated_at
      ...
      <div class="wf-external">
        <span class="wf-external__label"
          >外部输入（供 <code>&#123;&#123;user&#125;&#125;</code> 占位符引用）：</span
        >
        <input
          v-model="externalInput"
          class="wf-external__input"
          placeholder="例：帮我分析这个项目的技术栈"
        />
        <span class="wf-external__hint"
          >运行前填写，节点配置里写 &#123;&#123;user&#125;&#125; 即可引用此输入。</span
        >
      </div>
      ...
      <div class="wf-body">
        ...
        <div class="wf-palette">
          ...
          <button
            ...
            :style="{ '--t': WORKFLOW_NODE_COLORS.text }"
            @mousedown="onPaletteMouseDown('text', $event)"
            @click="onPaletteClick('text')"
          >
            <FileText :size="14" /> 文本
          </button>
          <button
            class="wf-palette__btn"
            :style="{ '--t': WORKFLOW_NODE_COLORS.llm }"
            @mousedown="onPaletteMouseDown('llm', $event)"
            @click="onPaletteClick('llm')"
          >
            <Bot :size="14" /> LLM
          </button>
          <button
            class="wf-palette__btn"
            :style="{ '--t': WORKFLOW_NODE_COLORS.tool }"
            @mousedown="onPaletteMouseDown('tool', $event)"
            @click="onPaletteClick('tool')"
          >
            <Wrench :size="14" /> 工具
          </button>
          <button
            class="wf-palette__btn"
            :style="{ '--t': WORKFLOW_NODE_COLORS.condition }"
            @mousedown="onPaletteMouseDown('condition', $event)"
            @click="onPaletteClick('condition')"
          >
            <GitBranch :size="14" /> 条件
          </button>
          <button
            class="wf-palette__btn"
            :style="{ '--t': WORKFLOW_NODE_COLORS.code }"
            @mousedown="onPaletteMouseDown('code', $event)"
            @click="onPaletteClick('code')"
          >
            <Code2 :size="14" /> 代码
          </button>
          <button
            class="wf-palette__btn"
            :style="{ '--t': WORKFLOW_NODE_COLORS.end }"
            @mousedown="onPaletteMouseDown('end', $event)"
            @click="onPaletteClick('end')"
          >
            <Flag :size="14" /> 结束
          </button>
          <select class="wf-palette__select" @change="loadTemplate">
            <option value="" disabled selected>📦 载入模板…</option>
            <option v-for="t in WORKFLOW_TEMPLATES" :key="t.id" :value="t.id">
        ...
        <div ref="canvasRef" class="wf-canvas" :class="{ 'wf-canvas--dragover': overCanvas }">
          <VueFlow
            id="workflow"
            v-model:nodes="nodes"
            v-model:edges="edges"
        ...
        <div class="wf-inspector">
          <template v-if="selectedEdgeWf && selectedEdgeIsCondition">
            ...
            <label class="wf-field"
              ><span>分支（true / false / 留空=始终）</span>
              <input
                :value="selectedEdgeWf?.label || ''"
                placeholder="true 或 false"
                @change="
                  (e: any) => {
                    if (selectedEdgeWf) selectedEdgeWf.label = e.target.value || undefined;
                  }
                "
              />
            </label>
            <div class="wf-palette__hint">
              该边从条件节点出发：填 true 表示条件成立时走此分支，填 false 相反。留空=始终激活。
            </div>
          </template>
          <template v-else-if="selectedWf">
            ...
            <template v-if="selectedWf.type === 'llm'">
              <label class="wf-field"
                ><span>提示词（支持 <code>&#123;&#123;id&#125;&#125;</code> 引用上游）</span
                ><textarea v-model="selectedWf.config.prompt" rows="4" />
              </label>
              <label class="wf-field"
                ><span>模型（可选，留空=默认）</span><input v-model="selectedWf.config.model"
              /></label>
            </template>
            <template v-else-if="selectedWf.type === 'tool'">
              <label class="wf-field"
                ><span>内置工具名</span><input v-model="selectedWf.config.tool"
              /></label>
              <div class="wf-field">
                <span>参数（JSON 对象，值可用占位符）</span>
                <textarea
                  :value="JSON.stringify(selectedWf.config.toolArgs || {}, null, 2)"
                  rows="5"
                  @change="
                    (e: any) => {
                      try {
                        updateToolArgs(JSON.parse(e.target.value));
                      } catch {
                        /* ignore */
                      }
                    }
            ...
            <template v-else-if="selectedWf.type === 'condition'">
              <div class="wf-field">
                <span>表达式（true/false 路由）</span>
                <textarea v-model="selectedWf.config.expression" rows="5" />
              </div>
              <div class="wf-palette__hint">
                支持 <code>&#123;&#123;id&#125;&#125;</code> 或裸节点 id 引用上游输出；运算符 == !=
                &gt; &lt; &gt;= &lt;= contains startsWith endsWith &amp;&amp; || ! （及 and or
                not）。例：<code>&#123;&#123;a&#125;&#125; contains "成功"</code>、<code
                  >score &gt; 80</code
                >。
              </div>
            </template>
            <template v-else-if="selectedWf.type === 'code'">
              <div class="wf-field">
                <span>JS 代码（入参 input/outputs，需 return）</span>
                <textarea v-model="selectedWf.config.code" rows="6" spellcheck="false" />
              </div>
              <div class="wf-palette__hint">
                函数体写法：<code>return input.trim().toUpperCase();</code>；也可
                <code>return outputs.a + outputs.b;</code>。对象自动 JSON 序列化。
              </div>
            </template>
            <template v-else-if="selectedWf.type === 'text'">
              <label class="wf-field"
                ><span>内容</span><textarea v-model="selectedWf.config.text" rows="4" />
              </label>
            </template>
            <button class="wf-btn wf-btn--danger" @click="removeNode(selectedWf.id)">
              <Trash2 :size="13" /> 删除节点
            </button>
            <!-- 运行状态 + 该步输出（节点级运行可视化） -->
            <template v-if="selectedWf.runStatus && selectedWf.runStatus !== 'waiting'">
              <div class="wf-field">
                <span
                  ...
                  <b :class="'wf-status-' + selectedWf.runStatus">
                    {{
                      selectedWf.runStatus === "running"
                        ? "⏳ 执行中"
                        : selectedWf.runStatus === "done"
                          ? "✅ 成功"
                          : selectedWf.runStatus === "error"
                            ? "❌ 失败"
                            : "⏭️ 跳过"
                    }}
                  </b>
              ...
              <div v-if="selectedWf.runOutput" class="wf-field">
                <span>本步输出</span>
                <pre class="wf-run__pre wf-run__pre--node">{{ selectedWf.runOutput }}</pre>
              </div>
            </template>
          </template>
          <div v-else class="wf-inspector__empty">
            点击节点/连线编辑配置<br />拖拽连线连接上下游
          </div>
        </div>
      </div>
      ...
      <div class="wf-run">
        ...
        <div class="wf-run__hist">
          ...
          <div v-for="r in runs" :key="r.id" class="wf-run__hist-item" :title="r.summary">
            <span class="wf-run__hist-status" :class="r.status === 'success' ? 'ok' : 'fail'">{{
              r.status === "success" ? "✓" : "✗"
            }}</span>
...
.wf-drag-preview {
  ...
  background: var(--bg-elevated);
  color: var(--text-primary);
  border: 1px solid var(--t, var(--accent-color));
  border-left: 3px solid var(--t, var(--accent-color));
...
.wf-drag-preview svg {
  color: var(--t, var(--accent-color));
}
.wf-canvas--dragover {
  border-color: var(--accent-color);
  box-shadow:
    inset 0 0 0 2px var(--accent-bg),
...
.wf-head__actions .wf-btn--run {
  background: var(--accent-color);
  border-color: var(--accent-color);
  color: #fff;
}
.wf-head__actions .wf-btn--run:hover {
  background: var(--accent-hover);
  border-color: var(--accent-hover);
  color: #fff;
}
.wf-head__actions .wf-btn--run:disabled {
  opacity: 0.6;
}
...
.wf-palette__btn {
  ...
  border-radius: var(--radius-sm);
  border: 1px solid var(--border-color);
  border-left: 3px solid var(--t, var(--border-strong));
  background: var(--bg-elevated);
  color: var(--text-primary);
  cursor: grab;
  font-size: 13px;
...
.wf-palette__btn:active {
  cursor: grabbing;
}
.wf-palette__btn:hover {
  transform: translateY(-1px);
  box-shadow: var(--shadow-md);
  border-color: var(--t, var(--border-strong));
  color: var(--t, var(--text-primary));
  background: var(--bg-hover);
}
.wf-palette__btn svg {
  flex-shrink: 0;
  color: var(--t, var(--text-secondary));
}
.wf-palette__hint {
  font-size: 11px;
  color: var(--text-muted);
...
.wf-run__hist-status.ok {
  background: #22c55e;
}
.wf-run__hist-status.fail {
