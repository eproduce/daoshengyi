...
<template>
  <div
    class="wf-node"
    :class="`wf-node--${data?.wf?.runStatus || 'waiting'}`"
    :style="{ '--c': COLORS[data?.wf?.type as WorkflowNodeType] || '#999' }"
  >
    <Handle
      id="in"
      type="target"
    ...
    <div class="wf-node__body">
      <div class="wf-node__row">
        <span class="wf-node__badge">{{
          TYPE_LABEL[(data?.wf?.type as WorkflowNodeType) || "text"]
        }}</span>
        <!-- 运行状态徽标 -->
        <span
          v-if="data?.wf?.runStatus && data?.wf?.runStatus !== 'waiting'"
          class="wf-node__run"
          :class="`wf-node__run--${data.wf.runStatus}`"
          :title="`运行：${data.wf.runStatus}`"
        >
    ...
    <template v-if="data?.wf?.type === 'condition'">
      <Handle
        ...
        :style="{ left: '30%' }"
        class="wf-handle wf-handle--true"
        title="true 分支"
      />
      <Handle
        id="false"
        ...
        :position="Position.Bottom"
        :style="{ left: '70%' }"
        class="wf-handle wf-handle--false"
        title="false 分支"
      />
    </template>
    <Handle
