...
function getMcpToolsPrompt(): string {
  ...
  const builtin =
    ...
    '\n- **kb_add** (app): 向知识库**录入一段文本/文档内容**（自动分块 + 关键词索引；本地嵌入模型可用时叠加语义向量）。参数 {"kb_name": "知识库名", "source": "来源/文档名（如 笔记.md）", "text": "要录入的完整文本内容"}。**使用时机**：用户给你一段文字/笔记要求「存进 XX 知识库」时调用；内容在文件里可先 read_file 读取再录入。' +
    ...
    '\n- **kb_search** (app): 在已索引的**知识库**中检索（关键词 + 中文分词；本地嵌入模型可用时叠加语义向量）。参数 {"kb_name": "知识库名", "query": "检索词", "limit": 可选条数（默认 6）}。**使用时机**：问题涉及知识库内容时先检索再作答；检索不到可换关键词。' +
