...
export function useMemorySystem() {
  ...
  async function maybeSummarize(
    ...
    // 检查是否已有摘要覆盖
    const existing = await invoke<SummaryRow[]>("get_summaries", { convId }).catch(
      () => [] as SummaryRow[],
    );
...
只返回 JSON 数组，不要其他内容。示例：[{"fact":"用户喜欢简洁的回答","type":"preference","importance":9}]`;
    ...
    try {
      ...
      for (const item of items) {
        ...
        const importance = Math.max(
          1,
          Math.min(10, item.importance ?? (item.type === "preference" ? 8 : 5)),
        );
        ...
        generateEmbedding(item.fact, cfg).then((emb) => {
          if (emb)
            invoke("set_fact_embedding", { id: mergedId || f.id, embedding: emb }).catch(() => {});
        });
  ...
  async function retrieveMemories(
    ...
    try {
      ...
      if (facts.length === 0 && config?.apiKey) {
        ...
        for (const kw of kws) {
          const more = await invoke<FactRow[]>("search_facts", {
            query: kw,
            limit: BigInt(Math.max(1, recallLimit - 2)),
          });
          for (const f of more) {
      ...
      if (config?.apiKey) {
        ...
        if (emb) {
          const scored = await invoke<[FactRow, number][]>("search_by_embedding", {
            embedding: emb,
            limit: BigInt(Math.min(3, recallLimit)),
          });
      ...
      const all = [
        ...prefs.filter((p) => p.importance >= 7 && !facts.some((f) => f.fact === p.fact)),
        ...facts,
      ];
      const seen = new Set<string>();
      const unique = all.filter((f) => {
  ...
  async function reviewMemories(config: {
    baseUrl: string;
    apiKey: string;
    model: string;
  }): Promise<string> {
    try {
      const all = await invoke<FactRow[]>("list_facts", { factType: "", limit: 200 }).catch(
        () => [] as FactRow[],
      );
  ...
  async function aggregateEpisodic(config: {
    baseUrl: string;
    apiKey: string;
    model: string;
  }): Promise<string> {
    try {
      const summaries = await invoke<SummaryRow[]>("list_all_summaries", { limit: 60 }).catch(
        () => [] as SummaryRow[],
      );
      if (summaries.length === 0)
        return "暂无会话摘要。对话达到一定长度会自动生成摘要，之后可在此跨会话汇总。";
