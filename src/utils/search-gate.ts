...
const WEB_INTENT =
  /(天气|新闻|股票|汇率|价格|最新|热点|资讯|排名|趋势|行情|政策|招聘|公司|产品|游戏|电影|事件|公告|教程|指南|怎么|如何|是什么|搜索|查询)/;
// 文档/附件处理类请求（转表格/生成文档/整理要点/解读证明/翻译/分析附件等）：
// 基于已给内容（附件/正文）本地即可完成，不需要联网；含明确联网意图词时不跳过
const DOC_EDIT =
  /(转成|转为|做成|整理|汇总|提取|解读|编辑|修改|总结|翻译|生成|分析).{0,10}(表格|文档|excel|word|pdf|md|清单|要点|报告|格式|证明|截图|文件|图片|附件)/i;
// 本地创作/生图类请求（画/绘制/创作/做图等）：用 HTML/CSS/SVG/代码本地即可完成，
// 无需联网搜索教程/素材（多模态视觉模型可理解画面）；含明确联网意图词（学教程/找参考/查素材）时不跳过
const ART_CREATE =
  /(画|绘制|画画|作画|画一幅|画一个|创作|插画|海报|logo|banner|漫画|表情包|配图|示意图|流程图|思维导图|数据图表|做一张|来一张|生成.{0,10}(图|图片|图像|插画|海报))/i;
