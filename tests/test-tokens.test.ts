...
assert(
  estimateTokens("你好 hello 世界") === 6,
  "中英混合 ≈ 6",
  `got ${estimateTokens("你好 hello 世界")}`,
);
...
assert(
  getPricing("deepseek-chat").inputMiss === 0.5 && getPricing("deepseek-chat").output === 2.0,
  "deepseek-chat 价格",
  `got ${JSON.stringify(getPricing("deepseek-chat"))}`,
);
assert(
  getPricing("deepseek-v4-flash").label === "DeepSeek V4 Flash",
  "deepseek-v4-flash 匹配",
  `got ${getPricing("deepseek-v4-flash").label}`,
);
assert(
  getPricing("deepseek-v4-flash").inputMiss === 3.0,
  "deepseek-v4-flash 输入价",
  `got ${getPricing("deepseek-v4-flash").inputMiss}`,
);
assert(
  getPricing("deepseek-v4-pro").output === 27.0,
  "deepseek-v4-pro 输出价",
  `got ${getPricing("deepseek-v4-pro").output}`,
);
assert(
  getPricing("deepseek-reasoner").inputMiss === 1.0,
  "deepseek-reasoner 价格",
  `got ${getPricing("deepseek-reasoner").inputMiss}`,
);
assert(
  getPricing("unknown-model").inputMiss === 3.0,
  "未知模型回退 V4 Flash 价",
  `got ${getPricing("unknown-model").inputMiss}`,
);
...
assert(Math.abs(c1b - 9.1) < 1e-6, "缓存命中 1M/1M ≈ ¥9.1", `got ${c1b}`);
